...
本项目旨在评估**移动端轻量级架构**在年龄估计任务上的实际效能。实验结果表明，在 AFAD 数据集（采用 Stratified Split 分层划分）上，基于 MobileNetV3 的改进模型实现了 **MAE 3.148**。在参数量仅为 5.4M 的前提下，该结果与 ResNet-18 等参数量更大的基准模型接近。
